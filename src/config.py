...
Как работает:
1. Загружает <корень проекта>/app_config/.env через python-dotenv
...
# Модуль лежит в src/, поэтому .env ищем на уровень выше: <корень>/app_config/.env
load_dotenv(Path(__file__).resolve().parent.parent / "app_config" / ".env")

CLIENT_ID = os.environ.get("CLIENT_ID")
CLIENT_SECRET = os.environ.get("CLIENT_SECRET")

if not CLIENT_ID or not CLIENT_SECRET:
    raise RuntimeError("CLIENT_ID и CLIENT_SECRET должны быть заданы в файле app_config/.env")
